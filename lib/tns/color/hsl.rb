...
module Color
  class HSL
    ...
    # :yields self:
    def initialize(h = 0, s = 0, l = 0, radix1 = 360.0, radix2 = 100.0, &block)
    ...
    # 25%, 35%)").
    def css_hsl
      format("hsl(%3.2f, %3.2f%%, %3.2f%%)", hue, saturation, luminosity)
    end

    # Present the colour as an HSLA (with alpha) HTML/CSS colour string (e.g.,
    # "hsla(180, 25%, 35%, 1)").
    def css_hsla
      format("hsla(%3.2f, %3.2f%%, %3.2f%%, %3.2f)", hue, saturation, luminosity, 1)
    end
    ...
    def inspect
      format("HSL [%.2f deg, %.2f%%, %.2f%%]", hue, saturation, luminosity)
    end
    ...
    def mix_with(color, mix_percent = 0.5)
      v = to_a.zip(coerce(color).to_a).map do |(x, y)|
        ((y - x) * mix_percent) + x
      end
