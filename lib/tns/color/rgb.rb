...
module TNS
  module Color
    class RGB
      ...
      # :yields self:
      def initialize(r = 0, g = 0, b = 0, radix = 255.0, &block)
      ...
      def pdf_fill
        format(PDF_FORMAT_STR, @r, @g, @b, "rg")
      end
      ...
      def css_rgb
        format("rgb(%3.2f%%, %3.2f%%, %3.2f%%)", red_p, green_p, blue_p)
      end
      ...
      def css_rgba(alpha = 1)
        format("rgba(%3.2f%%, %3.2f%%, %3.2f%%, %3.2f)", red_p, green_p, blue_p, alpha)
      end
      ...
      def to_xyz(color_space = :sRGB)
        raise ArgumentError, "Unsupported colour space #{color_space}." unless color_space.to_s.downcase == "srgb"

        # Inverse sRGB companding. Linearizes RGB channels with respect to
        # energy.
        r, g, b = [@r, @g, @b].map do |v|
          if v > 0.04045
      ...
      def to_lab(_color_space = :sRGB, reference_white = [95.047, 100.00, 108.883])
        xyz = to_xyz
      ...
      def delta_e94(color_1, color_2, weighting_type = :graphic_arts)
        ...
        _, a_1, b_1 = color_1.values_at(:L, :a, :b)
        _, a_2, b_2 = color_2.values_at(:L, :a, :b)
...
class << Color::RGB
  ...
  def by_name(name, &)
    __by_name.fetch(name.to_s.downcase, &)
  end
...
class << Color::RGB
  ...
  def __named_color(mod, rgb, *names)
    ...
    raise ArgumentError, "#{names.join(", ")} already defined in #{mod}" if used.length < names.length
